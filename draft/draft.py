from draft.board import Board as Board
from draft.adp_list import ADPList as ADPList
...
def single(Config):
        ...
        for i in range(1,Config.rounds+1):
                ...
                if i % 2 == 0:
                        for j in range(len(Config.pickers),0,-1):
                                ...
                                #print("picker type: %s, " % board.pickers[j-1].picker_type, "players: %s" % board.pickers[j-1].players)
        ...
        #for team in Config.pickers:
            #print("team %s, " % str(team), "total points %s" % Config.pickers[team].season_points)
...
@draft.command()
#@click.option('--position', type=int, help='This is the position that the neural network picker will train from.')
#@click.option('--randomize', default=0, help='This will cause the neural network picker to train from a different position each draft.')
#@click.option('--q', type=str, help='This is the file to export the Q matrix to after training.')
@draft_settings
def train(Config):

        """ This command will train a Q matrix using a Convolutional Neural Network (CNN). It will use a Q learning algorithm with epsilon-greedy
        action selection. It will train on 1,000,000 drafts and then save the trained Q matrix to a file designated by the '--q' option. """
        ...
        for x in range(1):
                Config.adp_list = ADPList(Config.year, Config.teams, Config.draft_format)
                players = picker.Picker.assign_pickers(Config.teams)

                names = pick_team_names(Config.teams)
                picker_vec = []

                #if randomize:
                players[random.randint(0,Config.teams-1)] = "train"
                #else:
                #        players[Config.position] = "train"

                for idx, val in enumerate(players):
                        picker_vec.append(picker.Picker.factory(players[idx], idx, Config.rounds, names[idx], Config.adp_list, '', ''))
                        #print(picker_vec[idx].picker_type)
                
                board = Board(Config.rounds, picker_vec, Config.adp_list)
                for i in range(1,Config.rounds+1):
                        ...
                        # odd rounds            
                                ...
                                for j in range(1,len(Config.pickers)+1):
                                        ...
                                        #board.poll_picker(i, j, j-1)
                                        board.poll_picker(i, j, j-1)
                # some kind of print statement at the end of the draft...
                index = players.index("train")
                print("Draft #%s:" % x, " total_points = %s, " % Config.pickers[index].season_points, "league rank = %s" % board.calculate_rank()[index])
                draft_data = [x, Config.pickers[index].season_points, board.calculate_rank(index)]
                export.append(draft_data)
        # export data somehow and possibly print final statistics
        data = pd.DataFrame(export, columns=['Draft #', 'Total Season Points', 'League Rank'])
        data.to_csv('draft/train_data.csv')
        index = players.index("train")
        json_Q_mat = json.dumps(Config.pickers[index].Q)

        with open("draft/Qmat.json", "w") as write_file:
                json.dump(json_Q_mat,write_file)

@draft.command()
@click.option('--position', required=True, type=int, help='This is the position to validate the trained Q matrix from.')
@click.option('--q', required=True, type=str, help='this is the file to read in the Q matrix from.')
@click.option('--randomize', default=0, help='this will randomize the position that the Q matrix picker will validate from.')
@draft_settings
def validate(Config):

        """ This command will import a Q matrix file that was trained using the 'draft train' command and will validate the 
        effectiveness of the trained Q matrix by simulating 100,000 drafts and recording the results. """
        ...
        for x in range(100000):
                ...
                if randomize:
                        players[numpy.random.randint(0,Config.teams-1)] = "smart"
                else:
                        players[Config.position] = "smart"

                for idx, val in enumerate(players):
                        picker_vec.append(picker.Picker.factory(players[idx], idx, rounds, names[idx], '', q, ''))
                        #print(picker_vec[idx].picker_type)
